...
class Ranking:
    ...
    def ranking_recursion(self, base_table_id: str, path: list, visited: set):
        ...
        features = [transform_node_to_dict(node) for node in base_table_node if
    ...
    def ranking_func_2(self, left_node, right_node, path: list, visited):
        ...
        left_table_features = list(left_table_df.drop(columns=[self.dataset.target_column]).columns)
        join_table_features = list(joined_df.drop(columns=[self.dataset.target_column]).columns)
        features_right = [feat for feat in join_table_features if feat not in left_table_features]

        # 2. Select dependent features
        dependent_features_scores = compute_correlation(left_table_features, joined_df, self.dataset.target_column)
