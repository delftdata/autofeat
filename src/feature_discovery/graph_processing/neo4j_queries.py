...
def _create_virtual_graph(tx, name):
    result = tx.run(
        "CALL gds.graph.project(" "$name, " "'Node', " "{ LINK: " "{ type: 'RELATED', " "orientation: 'UNDIRECTED'}})",
        name=name,
    )
...
def _enumerate_all_paths(tx, name):
    tx_result = tx.run(
        f"CALL gds.alpha.allShortestPaths.stream('{name}') "
...
def _get_adjacent_nodes_rels(tx, node_id):
    tx_result = tx.run(
        "match (n:Node {id: $node_id})-[r]-(m:Node) " "return m.id as id, collect(properties(r)) as props",
        node_id=node_id,
    )
    values = {}
    for record in tx_result:
