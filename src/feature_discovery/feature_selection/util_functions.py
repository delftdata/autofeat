...
from feature_discovery.experiments.understand_cife import (
    measure_conditional_dependency,
    measure_relevance,
)
from feature_discovery.feature_selection.feature_selection_algorithms import (
    FSAlgorithms,
)
...
def apply_feat_sel(joined_df, base_table_df, target_column, path):
    ...
    left_table_features = dict(
        filter(lambda x: x[1] in base_table_features, X_features.items())
    )
    right_table_features = dict(
        filter(
            lambda x: x[1] in joined_table_no_base.columns
            and x[1] not in base_table_features,
            X_features.items(),
        )
    )
    for alg in fs.ALGORITHM_FOREIGN_TABLE:
        print(f"Processing: {alg}")
        result[alg] = fs.feature_selection_foreign_table(
            alg,
            list(left_table_features.keys()),
            list(right_table_features.keys()),
            X,
            y,
        )
...
def compute_correlation(
    left_table_features: list, joined_df: pd.DataFrame, target_column: str
) -> dict:
    """
...
def compute_relevance_redundancy(
    ...
    to_drop_features = [
        ...
        if feat not in left_table_features and feat not in features_to_compare
    ]
    ...
    left_features = dict(
        filter(lambda x: x[1] in left_table_features, all_features.items())
    )
    right_features = dict(
        filter(lambda x: x[1] in features_to_compare, all_features.items())
    )

    fs = FSAlgorithms()
    scores = fs.feature_selection_foreign_table(
        fs.CIFE, list(left_features.keys()), list(right_features.keys()), X, y
    )
    # Map the scores to the columns
    ...
    redundancy_scores = measure_redundancy(
        list(left_features.keys()), list(right_features.keys()), X
    )
    redundant_features = dict(zip(list(right_features.values()), redundancy_scores))
    selected_redundant = get_elements_higher_than_value(
        redundant_features, redundancy_threshold
    ).keys()

    # Sort the scores in ascending order
    selected_non_redundant = {
        k: v for k, v in normalised_result.items() if k not in selected_redundant
    }
...
def _understand_metrics(left_features, right_features, X, y):
    ###
    redn = measure_redundancy(
        list(left_features.keys()), list(right_features.keys()), X
    )
    feat_red = dict(zip(list(right_features.values()), redn))
    cnd_dep = measure_conditional_dependency(
        list(left_features.keys()), list(right_features.keys()), X, y
    )
...
def measure_redundancy(selected_features, free_features, X):
    ...
    redundancy = np.vectorize(
        lambda free_feature: np.sum(
            matrix_mutual_information(arr_X[:, selected], arr_X[:, free_feature])
        )
    )(free)
