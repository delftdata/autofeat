...
from feature_discovery.config import (
    PLOTS_FOLDER,
    MAPPING_FOLDER,
    ACCURACY_RESULTS_ALL_PNG,
)
from feature_discovery.data_preparation.dataset_base import Dataset
from feature_discovery.experiments.arda_experiments import ArdaExperiment
from feature_discovery.experiments.base_experiment import BaseExperiment
from feature_discovery.experiments.datasets import Datasets
from feature_discovery.experiments.result_object import Result
from feature_discovery.experiments.tfd_experiments import TFDExperiment
from feature_discovery.helpers.util_functions import objects_to_dict
...
class AllExperiments:
    ...
    def experiments_per_dataset(self, dataset: Dataset):
        ...
        while len(queue) > 0:
            ...
            experiment = experiment_class(
                dataset, learning_curve_depth_values=self.learning_curves_depth_values
            )
    ...
    def plot_results(self, results_df: pd.DataFrame = None):
        ...
        for i, dataset in enumerate(self.datasets):
            ...
            results_df.sort_values(
                by=["approach", "algorithm"],
                key=lambda x: x.map(approach_plot_order).fillna(x),
                inplace=True,
            )
            results_df.to_csv(
                MAPPING_FOLDER / f"acc-results-{dataset.base_table_label}.csv",
                index=False,
            )

            if len(self.datasets) == 1:
                sns.barplot(
                    data=results_df, x="algorithm", y="accuracy", hue="approach", ax=axs
                )
    ...
    def plot_learning_curves(self):
        for dataset in self.datasets:
            ...
            fig.show()
            fig.savefig(
                PLOTS_FOLDER / f"learning-curves-{dataset.base_table_label}.png",
                dpi=300,
                bbox_inches="tight",
            )
