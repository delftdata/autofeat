...
from feature_discovery.algorithms import BaseAlgorithm, TRAINING_FUNCTIONS, CART
from feature_discovery.augmentation.ranking import Ranking, Rank
from feature_discovery.config import JOIN_RESULT_FOLDER, PLOTS_FOLDER
from feature_discovery.data_preparation.dataset_base import Dataset
from feature_discovery.data_preparation.utils import get_join_path, prepare_data_for_ml
from feature_discovery.experiments.base_experiment import BaseExperiment
from feature_discovery.experiments.result_object import Result
from feature_discovery.experiments.utils import hp_tune_join_all, map_features_scores
...
class TFDExperiment(BaseExperiment):
    ...
    def plot_sensitivity_result(self, results: pd.DataFrame = None):
        ...
        fig.show()
        fig.savefig(
            PLOTS_FOLDER / f"sensitivity-plots-{self.dataset.base_table_label}.png",
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def __process_joined_data_tfd(self, joined_df: pd.DataFrame, ranked_path: Rank):
        ...
        columns_to_drop = [
            ...
            if (c not in self.dataset.base_table_features)
            and (c not in ranked_path.features)
        ]
