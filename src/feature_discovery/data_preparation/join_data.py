...
from feature_discovery.config import JOIN_RESULT_FOLDER, ROOT_FOLDER
from feature_discovery.graph_processing.neo4j_transactions import (
    get_relation_properties,
    get_pk_fk_nodes,
)
from feature_discovery.helpers.util_functions import transform_node_to_dict


def join_tables_recursive(
    all_paths: dict,
    mapping,
    current_table,
    target_column,
    ...
    for table in all_paths[current_table]:
        ...
        if table not in path:
            join_path = join_tables_recursive(
                all_paths,
                mapping,
                table,
                target_column,
...
def join_and_save(
    ...
    left_table_df = pd.read_csv(
        ROOT_FOLDER / partial_join_path,
        header=0,
        engine="python",
        encoding="utf8",
        quotechar='"',
    ...
    right_table_df = pd.read_csv(
        ROOT_FOLDER / right_table_path,
        header=0,
        engine="python",
        encoding="utf8",
        quotechar='"',
    ...
    print(
        f"\tJoining {partial_join_path} with {right_table_path}\n\tOn keys: {from_col} - {to_col}"
    )
    joined_df = pd.merge(
        left_table_df,
        right_table_df,
...
def join_all(base_table_id: str):
    ...
    while len(queue) > 0:
        ...
        for pk, fk in nodes:
            ...
            partial_join = pd.merge(
                ...
                suffixes=("", fk_node["source_name"]),
            )
            modified_columns = {
                col.partition(fk_node["source_name"])[0]: col
                for col in partial_join.columns
                if col.endswith(fk_node["source_name"])
            }
    ...
    partial_join.drop(
        columns=list(set(foreign_keys).intersection(set(partial_join.columns))),
        inplace=True,
    )
...
def prune_or_join_2(
    left_path, right_path, left_key, right_key, join_result_name, prune_threshold=0.3
):
    # Read left side table
    left_table_df = pd.read_csv(
        left_path,
    ...
    # Verify join quality
    print(
        f"Verifying join quality:\n\t{left_path}\n\t\t{left_key}\n\t{right_path}\n\t\t{right_key}"
    )
    result = prune_table(
        left_table_df[left_key], right_table_df[right_key], prune_threshold
    )
    if result:
    ...
    print(
        f"\tJoining {left_path} with {right_path}\n\tOn keys: {left_key} - {right_key}"
    )
    joined_df = pd.merge(
        left_table_df,
        right_table,
...
def prune_or_join(
    ...
    right_table_df = pd.read_csv(
        ROOT_FOLDER / mapping[right_table_name],
        header=0,
        engine="python",
        encoding="utf8",
        quotechar='"',
    ...
    print(
        f"\tJoining {partial_join_path} with {right_table_name}\n\tOn keys: {left_key} - {right_key}"
    )
    joined_df = pd.merge(
        left_table_df,
        right_table,
...
def prune_table(left_column, right_column, null_threshold=0.3):
    ...
    null_ratio = sum([Counter(left_column)[el] for el in set_difference]) / len(
        left_column
    )
...
def join_scenario(left_df, right_df, left_join_col, right_join_col):
    ...
    if any([el > 1 for el in left_count.values()]) and any(
        [el > 1 for el in right_count.values()]
    ):
...
def enumerate_all(
    ...
    for node in nodes:
        ...
        for rel in related:
            ...
            path = enumerate_all(
                table_id, all_paths, cp.copy(), enumerated_paths, visited
            )
