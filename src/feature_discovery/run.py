...
from feature_discovery.experiments.ablation_experiments import ablation_study_enumerate_paths, \
    ablation_study_enumerate_and_join, ablation_study_prune_paths, ablation_study_feature_selection, \
...
def get_base_results(dataset: Dataset, autogluon: bool = True):
    ...
    dataframe = pd.read_csv(DATA_FOLDER / dataset.base_table_id, header=0, engine="python", encoding="utf8",
...
def get_classification_results(value_ratio: float):
    all_results = []
    ...
    pd.DataFrame(all_results).to_csv(RESULTS_FOLDER / f"all_results_autogluon.csv", index=False)
