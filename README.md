...
put it in [data/benchmark](data/benchmark) folder on this repository.
...
### Benchmark setting
1. [Download](https://surfdrive.surf.nl/files/index.php/s/vdlZIT70hZuoO8f) datasets and put them in [data/benchmark](data/benchmark).
2. Go to [config.py](src/feature_discovery/config.py) and set `DATASET_TYPE = "benchmark"
`
...
### Data Lake setting
1. Create database `lake` in neo4j:
```
create database lake
:use lake
``` 
2. Go to [config.py](src/feature_discovery/config.py) and set `NEO4J_DATABASE = 'lake'`
